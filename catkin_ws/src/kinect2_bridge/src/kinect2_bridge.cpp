...
private:
  ...
  bool initDevice(std::string &sensor)
  {
    ...
    cameraMatrixIr = cv::Mat::zeros(3, 3, CV_64F);
  ...
  void createCameraInfo()
  {
    ...
    cv::Mat colorMatArg = cv::Mat::zeros(3, 4, CV_64F);
    colorMatArg.at<double>(0, 0) = 1;
    colorMatArg.at<double>(1, 1) = 1;
    colorMatArg.at<double>(2, 2) = 1;

    cv::Mat IrMatArg = cv::Mat::zeros(3, 4, CV_64F);
    IrMatArg.at<double>(0, 0) = 1;
    IrMatArg.at<double>(1, 1) = 1;
    IrMatArg.at<double>(2, 2) = 1;

    cv::Mat lowResMatArg = cv::Mat::zeros(3, 4, CV_64F);
    lowResMatArg.at<double>(0, 0) = 1;
    lowResMatArg.at<double>(1, 1) = 1;
    lowResMatArg.at<double>(2, 2) = 1;
